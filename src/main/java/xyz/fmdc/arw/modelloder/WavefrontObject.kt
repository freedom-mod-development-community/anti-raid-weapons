...
class WavefrontObject(val resourceLocation: ResourceLocation) {
            ...
            while (reader.readLine().also { line = it } != null) {
                ...
                when (tokens[0]) {
                    ...
                    "g", "o" -> {
                        ...
                        currentGroup = groups.computeIfAbsent(groupName) { GroupObject(it) }
                    }
                    "f" -> {
                        if (currentGroup == null) {
                            currentGroup = groups.computeIfAbsent("default") { GroupObject(it) }
                        }
                        val face = Face()
    ...
    private class GroupObject(val name: String) {
